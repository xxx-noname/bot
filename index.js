...
const vcard = 'BEGIN:VCARD\n' //GANTI NO DAN NAMA DIBAWAH INI JADI PUNYA LU
            ...
            + 'FN:XxX official\n' //Credits
            + 'ORG: Furious Crew;\n' //MYBOT TEAM
            + 'TEL;type=CELL;type=VOICE;waid=6289524385697:+62 895-2438-5697\n' 
...
/******** OWNER NUMBER**********/
const ownerNumber = ["6289524385697@s.whatsapp.net"] //ganti nomor lu
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr code is ready. subscribe','white'),color('YOU','red'),color('TUBE','white'),color('XxX official','yellow'))
})
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ *XxX BOT BROADCAST*❯\n\n${body.slice(4)}`})
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 *XxX BOT BROADCAST* 」*\n\n${body.slice(4)}`)
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					}
					break
